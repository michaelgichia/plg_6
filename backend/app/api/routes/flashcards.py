...
Do not find questions outside the provided context, state that you don't know if you can't find. Do not use any external knowledge or information not present in the context.
...
def generate_flashcards_by_course_id(session: SessionDep, current_user: CurrentUser, id: uuid.UUID) -> List[QAItem]:
    ...
    # We need to get the index name from below
    # if course:
    #     statement = (
    #         select(Document)
    #         .join(Course)
